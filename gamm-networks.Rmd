...
```{r}
m2 <- gam(similarity ~ s(distance, by = water) + water + from + s(x, y),
          method = 'REML',
          select = TRUE,
          family = betar(),
          data = pots_dat)
...
```{r}
uk_pts <- uk_boundary %>%
  st_make_grid(cellsize = 0.03, what = 'centers') %>%
  st_intersection(uk_boundary) %>%
  st_coordinates %>%
  as_tibble %>%
  rename(x = X, y = Y) 

#bind_rows(uk_pts, .id = 'from') %>%
uk_pts %>%
  mutate(from = factor(1, levels = c('1', '2')),
         distance = 50,
         water = FALSE) %>%
...
ggplot() +
  ...
  geom_sf(data = uk_boundary, fill = NA) +
  facet_wrap(~from) +
  #geom_sf(data = filter(pots_sites, site == 'Oxford'), size = 4) +
  labs(title = 'Ceramics in Roman Britain', subtitle = 'Distribution of ceramics produced in Oxford') +
  coord_sf(datum = NA, xlim = c(-6, 2), ylim = c(50, 53.7)) +
  theme_void()
```
```{r}
...
ggraph() +
  ...
  geom_edge_link(aes(colour = percent), edge_width = 2, lineend = 'round') +
  geom_sf(data = filter(pots_sites, site == 'Oxford'), size = 4) +
  scale_edge_color_distiller(palette = 'YlGnBu', guide = 'legend', name = 'Percent Oxford \nwares') +
  labs(title = 'Ceramics in Roman Britain', subtitle = 'Distribution of ceramics produced in Oxford') +
  coord_sf(datum = NA, xlim = c(-6, 2), ylim = c(50, 53.7)) +
  theme_void()
