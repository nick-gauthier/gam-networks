...
```{r}
iriquois_dat <- full_join(lcp_groups, br_groups) %>%
...
```{r}
m1 <- gam(similarity ~ s(lcp), data = iriquois_dat, method = 'ML')
m2 <- gam(similarity ~ s(lcp), family = betar(), data = iriquois_dat, method = 'ML')
m3 <- gamm(qlogis(similarity) ~ s(lcp), data = iriquois_dat, correlation = corMLPE(form = ~from + to), method = 'ML')
