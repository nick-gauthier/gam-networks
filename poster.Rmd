...
Plenty! In archaeology, they're particuarly useful for fitting so-called ``gravity'' style spatial interaction models. Most archaeological network data proxy for some kind of *flow* -- goods, people, information -- between spatially-structured populations. Even aspatial network data can be modeled using the same basic structure (actor models):
...
```{r}
vis.gam(m1$gam, view = c('distance', 'population'), theta = 40, phi = 20, type = 'response', se = -1, color = 'bw')
```
...
We can model this data in R.
$logit\left(proportion\right) = f\left(distance\right) + origin$
