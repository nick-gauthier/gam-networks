...
```{r, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, results = 'asis', out.width = "80%", warning = FALSE, fig.align = 'center', message = FALSE, dev='cairo_pdf')
```
...
Generalized additive mixed models (GAMMs) are a flexible form of regression model well-matched to the complexities of the archaeological record, including non-normal distributions such as counts or proportions, non-linear functional relationships, and non-independent observations with correlated errors.
...
Plenty! In archaeology, they're particuarly useful for fitting so-called ``gravity'' style spatial interaction models. Most archaeological network data proxy for some kind of *flow* -- goods, people, information -- between spatially-structured populations.:
...
The tricky part comes when we actually have to define all the $f\left(\right)$s. This is where splines come in handy.

# How do they work?

Penalized plines are the secret sauce that make GAMs so useful. They allow us estimate smooth functional forms directly from the data, rather than trying to fit the data to some prespecified function. They minimize overfitting by penalizing the ``wiggliness'' of the function.

\begin{center}
\includegraphics{Figures/spline1.png}
\end{center}
...
A dataset of Late Romano-British pottery. We have data for the percentage of the ceramic assemblage at 30 sites that were produced at Oxford or New Forest, and want to understand something about how these pots were moved over space.
```{r}
data("OxfordPots")
pots <- OxfordPots %>%
  rename(to = Place) %>%
...
```{r, eval=FALSE}
#By examining the residuals of the distance only spatial interaction model, we can isolate network links that are unusually strong or weak than we would expect if distance alone were influencing interaction.
pots_net %E>%
...
Scan this QR code to access a Github repository with the code used to generate this poster, as well as a mored detailed guide to analyzing network data with GAMMs.

\begin{center}
\includegraphics[width=.2\columnwidth]{Figures/qr}
\end{center}
